...
if [[ "$SINGLE_SHOT_TRIALS" -ne 50 || "$REFINED_TOTAL_NEW_TRIALS" -ne 50 ]]; then
  echo "Expected 50 total new trials, got single=$SINGLE_SHOT_TRIALS refined=$REFINED_TOTAL_NEW_TRIALS" >&2
  exit 1
fi
