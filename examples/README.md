...
Submit the equal-budget nanobench ablation on Slurm with `sbatch examples/nanochat/run_nanobench_ablation.sbatch`. It requests four GPUs on one node and runs the four discovery arms concurrently, with one GPU and an isolated compilation cache per arm; finalist validation starts after all four finish. Each discovery submission atomically claims a fresh `RUN_GROUP`/`OUT_ROOT`, and the timestamped defaults provide one. If only validation is interrupted, resubmit with the original `OUT_ROOT`, `VALIDATION_ONLY=1`, and a one-GPU `sbatch` resource override. The Centaur arm defaults to 100 trials, a 10-trial CMA-ES warmup, LLM proposal probability 0.3, and scheduler seed 42; override these with `CENTAUR_TRIALS`, `CENTAUR_WARMUP_TRIALS`, `CENTAUR_LLM_PROBABILITY`, and `CENTAUR_SEED`.
